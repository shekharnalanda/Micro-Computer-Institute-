...
<header class="site-header"><a class="brand" href="#home"><img class="brand-logo" src="{{ asset('images/mci-logo.webp') }}" alt="MCI logo"><span><strong>Micro Computer</strong><small>Institute</small></span></a><button class="mobile-menu-toggle" id="mobileMenuToggle" type="button" aria-expanded="false" aria-controls="siteNavigation" aria-label="Open navigation"><span></span><span></span><span></span></button><nav id="siteNavigation">why">Why MCI</a>#home">Home</a><a href="#courses">Courses</a><a href="#notices">Notices</a><a href="#gallery">Gallery</a><a href="#why">Why MCI</a><a href="#jobs">Job Search</a><a href="{{ route('certificates.verify') }}">Verify Certificate</a><a href="{{ route('student.login') }}">Student Login</a><a href="#enquiry">Enquiry</a><a href="#contact">Contact</a><a class="admin-nav-link" href="{{ route('admin.login') }}">Admin Login</a></nav><a class="pill" href="{{ route('admission.create') }}">Apply Online ↗</a></header>
...
<div class="notice-public-grid">@foreach(array_slice($notices,0,6) as $notice)<article class="notice-public-card type-border-{{ $notice['type'] }}"><div class="notice-public-top"><span class="notice-public-type">{{ strtoupper($notice['type']) }}</span><time>{{ \Carbon\Carbon::parse($notice['notice_date'])->format('d M Y') }}</time></div><h3>{{ $notice['title'] }}</h3>@if($notice['title_hi'])<h4>{{ $notice['title_hi'] }}</h4>@endif @if($notice['description'])<p>{{ $notice['description'] }}</p>@endif @if($notice['link'])why">Why MCI</a>{{ $notice['link'] }}" target="_blank" rel="noopener">View details ↗</a>@endif @if($notice['is_pinned'])<b class="public-pin">★ Important</b>@endif</article>@endforeach</div>
</section>
@endif
@if(count($gallery))
