...
<div><div class="resource-meta"><b>{{ $resource['course_code'] }}</b><span class="visibility-{{ ($resource['is_active'] ?? true) ? 'on' : 'off' }}">{{ ($resource['is_active'] ?? true) ? 'Visible' : 'Hidden' }}</span>@if($resource['due_date'])<span>Due {{ \Carbon\Carbon::parse($resource['due_date'])->format('d M Y') }}</span>@endif</div><h3>{{ $resource['title'] }}</h3><p>{{ $resource['description'] ?: 'No additional instructions.' }}</p><a class="resource-open" href="{{ !empty($resource['file_path']) ? route('admin.learning.download',$resource['id']) : $resource['link_url'] }}" target="_blank" rel="noopener noreferrer">{{ !empty($resource['file_path']) ? 'Download PDF ↓' : 'Open Resource ↗' }}</a></div>
