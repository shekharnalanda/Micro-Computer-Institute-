...
.inner{display:flex;flex-direction:column;align-items:center;justify-content:space-between;height:100%;padding:18px 48px 16px;-webkit-print-color-adjust:exact;print-color-adjust:exact}
.certificate-header{flex:0 0 auto}.award-block{flex:0 0 auto}.verification-block{flex:0 0 auto}.logo{width:80px;height:80px}
...
.recipient-row{width:74%;max-width:830px;grid-template-columns:82px minmax(0,1fr) 82px;gap:24px;margin:8px auto}
.student-photo{width:72px;height:86px}
.verify-qr{width:72px;height:72px}
.verify{font-size:9px}
.seal{width:90px;height:90px;font-size:15px}
.meta{gap:18px;margin-top:0;font-size:10px}
.signatures{width:100%;margin-top:0;font-size:11px}
.signatures span{min-width:190px}
.signatures .seal{flex:0 0 auto}
}
...
<article class="certificate"><div class="inner">@php($certificateQr=urlencode(route('certificates.verify').'?code='.$certificate['verification_code']))<section class="certificate-header"><img class="logo" src="{{ asset('images/mci-logo.webp') }}" alt="MCI"><p class="eyebrow">MICRO COMPUTER INSTITUTE</p><h1>{{ $certificate['title'] }}</h1><div class="subtitle">{{ $certificate['type']==='completion' ? 'Certificate of Achievement' : ($certificate['type']==='merit' ? 'Recognition of Excellence' : 'Recognition of Participation') }}</div></section><section class="award-block"><p class="presented">This certificate is proudly presented to</p><div class="recipient-row"><div>@if(!empty($student['photo_path']))<img class="student-photo" src="{{ asset($student['photo_path']) }}" alt="{{ $student['student_name'] }}">@else<div class="student-photo student-photo-fallback">{{ strtoupper(substr($student['student_name'],0,1)) }}</div>@endif<small class="photo-caption">STUDENT PHOTO</small></div><h2 class="student">{{ $student['student_name'] }}</h2><div class="verify"><img class="verify-qr" src="https://api.qrserver.com/v1/create-qr-code/?size=160x160&amp;data={{ $certificateQr }}" alt="Certificate verification QR">Verify Online<b>{{ $certificate['verification_code'] }}</b></div></div>
...
</section><section class="verification-block"><div class="meta"><span>Certificate No: <b>{{ $certificate['certificate_no'] }}</b></span><span>Issue Date: <b>{{ \Carbon\Carbon::parse($certificate['issue_date'])->format('d F Y') }}</b></span><span>Roll No: <b>{{ $student['roll_no'] ?? $student['application_no'] }}</b></span></div></section><div class="signatures"><span>Course Coordinator</span><div class="seal">MCI<br>VERIFIED</div><span>Authorised Signatory</span></div></div></article></body></html>
