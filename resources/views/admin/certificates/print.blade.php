...
.credential-row{gap:48px;margin:8px auto 8px}
.student-photo{width:76px;height:92px}
.verify-qr{width:76px;height:76px}
.verify{font-size:9px}
.seal{width:90px;height:90px;font-size:15px}
.meta{gap:38px;margin-top:8px;font-size:12px}
...
</section><section class="verification-block"><div class="credential-row"><div>@if(!empty($student['photo_path']))<img class="student-photo" src="{{ asset($student['photo_path']) }}" alt="{{ $student['student_name'] }}">@else<div class="student-photo student-photo-fallback">{{ strtoupper(substr($student['student_name'],0,1)) }}</div>@endif</div><div class="seal">MCI<br>VERIFIED</div><div class="verify"><img class="verify-qr" src="https://api.qrserver.com/v1/create-qr-code/?size=160x160&amp;data={{ $certificateQr }}" alt="Certificate verification QR">Verify Online<b>{{ $certificate['verification_code'] }}</b></div></div><div class="meta"><span>Certificate No: <b>{{ $certificate['certificate_no'] }}</b></span><span>Issue Date: <b>{{ \Carbon\Carbon::parse($certificate['issue_date'])->format('d F Y') }}</b></span><span>Roll No: <b>{{ $student['roll_no'] ?? $student['application_no'] }}</b></span></div></section><div class="signatures"><span>Course Coordinator</span><span>Authorised Signatory</span></div></div></article></body></html>
